...
#define MAX_RETRIES 25   // Task 2: Allow several retransmission attempts for packets
...
void *client_thread_func(void *arg) 
{
    ...
    for (int request_index = 0; request_index < num_requests; request_index++) 
    {
        ...
        {
            // Record timestamp of start of request so we can calculate RTT later
            ...
            if (num_ready_fds > 0 && FD_ISSET(client_thread_data->socket_fd, &read_fds)) 
            {
                ...
                if (bytes_received >= (int)sizeof(ack_frame)) 
                {
                    ...
                    {
                        // ACK is valid, so calculate the RTT using gettimeofday difference between time sent and time received in microseconds
                        ...
                        // Task 2: Packet received successfully. Increment received packets, mark packet as acknowledged to stop retry loop, and toggle sequence number (0 or 1)
                        rx_cnt++;
                        acknowledged = true;
