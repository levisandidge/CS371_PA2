...
typedef struct {
    ...
    long total_messages;
    float request_rate;
    int thread_number;  // Task 1 - Determine the packet loss for each individual thread
...
{
    // Setup data and event structs, message being sent by the client, and time evaluations of message being sent and received
    ...
    for (int request_index = 0; request_index < num_requests; request_index++) 
    {
        ...
        //Fixme? (if we want no timeout) int number_of_triggered_events = epoll_wait(client_thread_data->epoll_fd, triggered_events, MAX_EVENTS, -1); 
...
void run_client() 
{
    ...
    {
        // Task 1: Specify the thread number
...
void run_server() 
{
    ...
    while (1) 
    {
        ...
        if (ret > 0) 
        {
            ...
            // Report the message echoed by the server
