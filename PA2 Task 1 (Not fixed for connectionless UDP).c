...
void *client_thread_func(void *arg) 
{
    ...
    for (int request_index = 0; request_index < num_requests; request_index++) 
    {
        ...
        // Fixme int number_of_triggered_events = epoll_wait(client_thread_data->epoll_fd, triggered_events, MAX_EVENTS, 1000); 
        if (number_of_triggered_events > 0 && triggered_events[0].data.fd == client_thread_data->socket_fd) 
        {
            ...
            while (total_bytes_received < MESSAGE_SIZE) 
            {
                ...
                // SUPER IMPORTANT (this is why packet loss works now)

                // Only count the message as received if we received the correct response
                ...
                {
                    // Increment the lost packet count if the message was incomplete or not received properly
                    lost_pkt_cnt++;
    ...
    while (1) 
    {
        ...
        for (int i = 0; i < n; i++) 
        {
            ...
            else 
            {
                ...
                // Task 1: Need client address length for UDP for server to retrieve data from clients
                socklen_t client_address_length = sizeof(client_address);

                // Task 1: Use recvfrom instead of recv to receive data from clients
                ...
                if (ret > 0) 
                {
                    // Task 1: Use sendto instead of send when echoing the message back to the client
